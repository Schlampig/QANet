...
class Model(object):
    def __init__(self, config, batch, word_mat=None, char_mat=None, trainable=True, opt=True, demo=False, graph=None):
        ...
        with self.graph.as_default():
            ...
            if opt:
                ...
                self.c = tf.slice(self.c, [0, 0], [N, self.c_maxlen])  # tf.slice 从[0, 0]开始取值，取到前N个向量，每个取到maxlen长度
    ...
    def forward(self):
        config = self.config
        # N-batch size, PL-context max length, QL-query max length, CL-char limit,
        # d-hidden number, dc-char dimension, nh-number of heads
        ...
        with tf.variable_scope("Context_to_Query_Attention_Layer"):
            ...
            self.c2q = tf.matmul(S_, q)  # 论文中的A=S*Q^T，context to question attention
            self.q2c = tf.matmul(tf.matmul(S_, S_T), c)  # 论文中的B=S*S_^T*C^T，question to context attention
        ...
        with tf.variable_scope("Output_Layer"):
            start_logits = tf.squeeze(conv(tf.concat([self.enc[1], self.enc[2]], axis=-1),
                                           1, bias=False, name="start_pointer"), -1)
            end_logits = tf.squeeze(conv(tf.concat([self.enc[1], self.enc[3]], axis=-1),
