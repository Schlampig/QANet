...
def dot_product_attention(q, k, v, bias, mask=None, scope=None, reuse=None, dropout=0.0):
    with tf.variable_scope(scope, default_name="dot_product_attention", reuse=reuse):
        ...
        weights = tf.nn.dropout(weights, 1.0 - dropout)  # dropping out the attention links for each of the heads
...
norm_fn = layer_norm  # tf.contrib.layers.layer_norm
...
def conv_block(inputs, num_conv_layers, kernel_size, num_filters, scope="conv_block",
...
def multihead_attention(queries, units, num_heads, memory=None, seq_len=None, scope="Multi_Head_Attention",
                        reuse=None, mask=None, is_training=True, bias=True, dropout=0.0):
    with tf.variable_scope(scope, reuse=reuse):
        # Self attention
        if memory is None:
            memory = queries
        ...
        x = dot_product_attention(Q, K, V, bias=bias, mask=mask, scope="dot_product_attention", reuse=reuse, dropout=dropout)
        return combine_last_two_dimensions(tf.transpose(x,[0,2,1,3]))


def self_attention_block(inputs, num_filters, seq_len, mask=None, num_heads=8, scope="self_attention_ffn",
                         reuse=None, is_training=True, bias=True, dropout=0.0, sublayers=(1, 1)):
    with tf.variable_scope(scope, reuse=reuse):
        l, L = sublayers
        # Self attention
        outputs = norm_fn(inputs, scope="layer_norm_1", reuse=reuse)
        outputs = tf.nn.dropout(outputs, 1.0 - dropout)
        outputs = multihead_attention(outputs, num_filters, num_heads=num_heads, seq_len=seq_len, reuse=reuse,
...
def residual_block(inputs, num_blocks, num_conv_layers, kernel_size, mask=None, num_filters=128,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_blocks):
            ...
            outputs, sublayer = conv_block(outputs, num_conv_layers, kernel_size, num_filters, reuse=reuse,
                                           dropout=dropout, scope="encoder_block_%d" % i,
            ...
            outputs, sublayer = self_attention_block(outputs, num_filters, seq_len, mask=mask, num_heads=num_heads,
                                                     scope="self_attention_layers%d" % i, reuse=reuse, bias=bias,
